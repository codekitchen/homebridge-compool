...
class CompoolController {
	...
	configurePoolSpa(name, capName) {
		...
		if (acc) {
			...
			temp.getCharacteristic(Characteristic.TargetTemperature)
				.on('set', (value, cb) => this.controller[`set${capName}Temp`](value, cb));
			temp.getCharacteristic(Characteristic.TargetHeatingCoolingState)
				.setProps({ validValues: [0, 1, 2], maxValue: 2 })
				.on('set', (value, cb) => this.setHeatingState(acc, value, `set${capName}Heater`, cb));
			if (name == 'spa') {
	...
	setHeatingState(acc, value, setHeater, cb) {
		let setPumpTo = false;
		...
		let setAux = () => {
			if (this.getAuxStatus(acc.context.pump) != setPumpTo) {
				this.controller.toggleAux(acc.context.pump, cb);
			} else {
				cb();
			}
		};
		if (setHeaterTo !== null) {
			this.controller[setHeater](setHeaterTo, setAux);
		} else {
			setAux();
		}
	}
