...
class CompoolController {
    ...
    async setHeatingState(acc, value, setHeater) {
        ...
        if (this.getAuxStatus(acc.context.pump) != setPumpTo) {
            await promisify(this.controller.toggleAux.bind(this.controller))(acc.context.pump);
        }
        if (setHeaterTo !== null) {
            await promisify(this.controller[setHeater].bind(this.controller))(setHeaterTo);
        }
    }
    ...
    refreshAll() {
        ...
        if (this.config.timezone) {
            ...
            if (skew > 3 && skew < (22 * 60)) { // avoid midnight gap
                ...
                this.controller.setTime(curTime.getHours(), curTime.getMinutes(), () => { });
            }
        }
    }
